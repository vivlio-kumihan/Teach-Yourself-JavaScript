...
|hasAttribute()|属性を持っているかを確認する。何らかの属性を保持している => true,　保持していない => falseを返す。|
|hasAttribute(name)|指定した引数の属性を持っているかを確認する。何らかの属性を保持している => true,　保持していない => falseを返す。|

### clone, contents, dateset
```html
<main class="parent">
  <p class="parent__child order-1" data-color="purple">1. JavaScript</p>
  <p class="parent__child order-2" data-color="red">2. React.js</p>
  <p class="parent__child order-3" data-color="orange">3. Next.js</p>
</main>
```
